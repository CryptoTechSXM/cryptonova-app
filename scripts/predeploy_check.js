...
const ADDR_FILE = process.env.ADDRESSES_FILE || "deployed_addresses_v8_22.json";
...
if (deployText) {
  ...
  // Check ADDRESSES_FILE default is v8_22
  if (deployText.includes("deployed_addresses_v8_22.json")) {
    ok("deploy_v8.js output file: deployed_addresses_v8_22.json");
  } else {
    fail("deploy_v8.js does not output to deployed_addresses_v8_22.json");
  }
...
// ─────────────────────────────────────────────────────────────────────────────
sep("deployed_addresses_v8_22.json — presence check");
{
  const addrFile = path.join(ROOT, "deployed_addresses_v8_22.json");
  if (!fs.existsSync(addrFile)) {
    console.log("  ℹ  deployed_addresses_v8_22.json not found — run deploy_v8.js first");
  } else {
    ...
    for (const key of required) {
      ...
      else               fail(`addresses: ${key} missing from deployed_addresses_v8_22.json`);
    }
