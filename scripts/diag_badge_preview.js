...
(async () => {
  ...
  console.log("  `node scripts\\diag_eviction_clock.js` counts the SAME thing by the same rule.");
  console.log("");
  console.log("  ** BUT THAT COMPARISON IS CONFOUNDED WHILE BIGFILL IS RUNNING, AND THE FIRST");
  console.log("     ATTEMPT AT IT WAS. ** Measured 2026-08-19: this script run twice, 20 SECONDS");
  console.log("  apart, scanned 109 then 110 parked positions. The population changes on a scale of");
  console.log("  seconds, so two scripts run minutes apart are NOT looking at the same population and");
  console.log("  a difference of a few between them is drift, not a defect. Readings that evening:");
  console.log("  37/107, 41/108, 40/109, 40/110 - the uncoverable count moves more slowly than the");
  console.log("  population does, which is what you would expect if new parkers arrive rescuable and");
  console.log("  the uncoverable set is the accumulated hard cases.");
  console.log("  TO COMPARE THE TWO INSTRUMENTS HONESTLY: stop the bigfill loop first, or compare");
  console.log("  only the PER-MEMBER rows (gap, headroom, verdict) which must agree to the cent on");
  console.log("  every member both list. THAT is the check with no drift in it. A per-member");
  console.log("  disagreement is a real finding; a difference in the totals alone is not.");
  console.log("");
