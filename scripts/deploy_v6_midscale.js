/**
 * deploy_v6_midscale.js — CryptoNova V6 Midscale Self-Test Deploy
 * ─────────────────────────────────────────────────────────────────
 * Full 127-member BFS matrix with quickfill support.
 * Use quickfill.js to auto-fill positions after registering real wallets.
 *
 * Economics per cycle (root with 6 directs, 127-member matrix):
 *   L1 referrals: 6 × $2.50 = $15.00
 *   Chain pay (126 members below root):
 *     L1 (2 members): 2 × $2.00 = $4.00
 *     L2 (4 members): 4 × $0.80 = $3.20
 *     L3 (8 members): 8 × $0.30 = $2.40
 *     L4-L7 (112 members): varies ≈ $8-12
 *   Total per cycle: ~$32-35 — easily covers $10 re-entry + $25 T2 upgrade
 *
 * Run: npx hardhat run scripts/deploy_v6_midscale.js --network baseSepolia
 * Then: npx hardhat run scripts/quickfill.js --network baseSepolia
 */
"use strict";
const { ethers } = require("hardhat");
require("dotenv").config();
const { PARAMS, TIER_FEE_MULTIPLIERS, TIER_NAMES, USDC_UNIT } = require("./v6_params");
const P = PARAMS.midscale;
...
async function main() {
  ...
  sep();
  console.log(`  🔥  CryptoNova V6 MIDSCALE DEPLOY`);
  console.log(`  ${P.label}`);
  console.log(`  MATRIX=${P.MATRIX_SIZE} | BELT_MAX=${P.BELT_MAX} | $10 | quickfill ready`);
  sep();
  ...
  for (let t = 1; t <= 7; t++) {
    ...
    if (t === 1) await cnova.grantRole(await cnova.EPOCH_ROLE(), await belts[0].getAddress());
  ...
  sep();
  console.log("  🔥 V6 MIDSCALE DEPLOY COMPLETE");
  sep();
  ...
  console.log(`  TierManagerV6   : ${tmAddr}`);
  for (let t = 1; t <= 7; t++) {
    console.log(`  T${t} BeltManager  : ${await tierSystems[t].bm.getAddress()}  ($${TIER_FEE_MULTIPLIERS[t]} ${TIER_NAMES[t]})`);
  }
  sep();
  console.log(`  Matrix size     : ${P.MATRIX_SIZE} (FULL BFS 7-level tree)`);
  console.log(`  Belt max        : ${P.BELT_MAX}`);
  console.log(`  Belts per tier  : ${P.NUM_BELTS}`);
  console.log(`  Epoch limit     : ${P.EPOCH_MEMBER_LIMIT}`);
  console.log(`\n  Next step: npx hardhat run scripts/quickfill.js --network baseSepolia`);
  sep();
}
