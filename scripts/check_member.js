...
// ── Dee1's wallet ──────────────────────────────────────────────
const WALLET = '0xa56cfcde5d539f95c22d723c0ffca5fadf6b47f4';
...
// Member struct (V8.30): id, referrer, joinedAt, withdrawable,
// totalEarned, totalWithdrawn, cyclesCompleted, isInMatrix, hasEverJoined
const MAT_ABI = [
...
async function main() {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`  Dee1 wallet diagnostic — V8.30`);
  console.log(`  ${WALLET}`);
  ...
  const opts  = await router.getMemberOptions(WALLET);
  ...
  for (const t of TIERS) {
    for (const [label, addr] of [['MatA', t.matA], ['MatB', t.matB]]) {
      try {
        ...
        // Member has never interacted with this matrix — skip silently
      }
    }
  }
