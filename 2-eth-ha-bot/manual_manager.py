"""
manual_manager.py — ETH HA Bot V1 (Manual Trade Manager)
...
How to use:
  1. Open a trade manually in MT5 on ETHUSD.
...
class ManualTradeManager:
    """
    Attaches to any open manual position on ETHUSD and manages it
    ...
    def scan_and_manage(self):
        ...
        Finds all open ETHUSD positions and manages each one.
        ...
        if not positions:
            print("📭 No open positions on ETHUSD")
            return 0
    ...
    def _attach(self, position, df_m5):
        ...
        print(f"\n🔗 Attaching to manual {direction} position #{ticket} @ {entry:.4f}")
        ...
        if needs_sl or needs_tp:
            ...
            print(f"   {'SL' if needs_sl else ''}{'/' if needs_sl and needs_tp else ''}{'TP' if needs_tp else ''} set → "
                  f"SL: {new_sl:.4f}  TP: {new_tp:.4f}  (ATR: {current_atr:.4f})")
        else:
            print(f"   Position already has SL: {position.sl:.4f}  TP: {position.tp:.4f} — keeping them")
    ...
    def _manage(self, position, df_m1, df_m5):
        ...
        if not state["be_triggered"]:
            if direction == "BUY":
                ...
                if profit_so_far > current_atr and position.sl < be_sl:
                    print(f"🟡 Break-even BUY #{ticket} → SL to {be_sl:.4f} (entry + spread)")
                ...
                if profit_so_far > current_atr and position.sl > be_sl:
                    print(f"🟡 Break-even SELL #{ticket} → SL to {be_sl:.4f} (entry - spread)")
                    modify_sl(position, be_sl)
                    state["be_triggered"] = True
        ...
        if direction == "BUY":
            ...
            if new_sl > position.sl:
                print(f"🔒 Trail BUY #{ticket} → SL to {new_sl:.4f}")
                modify_sl(position, new_sl)

        else:
            new_sl = current_price + (current_atr * config.TRAILING_ATR_MULTIPLIER)
            if new_sl < position.sl:
                print(f"🔒 Trail SELL #{ticket} → SL to {new_sl:.4f}")
                modify_sl(position, new_sl)
    ...
    def _exit(self, position, direction, entry_price, current_price):
        ...
        # FIXED: capture position.profit (real USD P&L) BEFORE closing.
        # Raw price difference is not the same as USD P&L.
