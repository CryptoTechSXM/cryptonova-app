...
def atr(df, period=14):
    """
    Average True Range — measures how much ETH typically moves per candle.
    We use ATR to set SL and TP distances that adapt to current volatility
    instead of fixed values that go stale.
    ETH ATR in absolute USD terms is lower than BTC — MIN_ATR in config
    is set accordingly (3.0 vs 50.0 for BTC).
    """
    df = df.copy()
...
def is_doji(candle, threshold=0.15):
    ...
    if range_ == 0:
        return False
    ratio = body / range_
    print(f"🧪 Doji → Body:{body:.4f} Range:{range_:.4f} Ratio:{ratio:.4f} (max:{threshold})")
...
def is_high_volume_doji(df_m1, lookback=3):
    ...
    print(f"📊 Doji size: {doji_range:.4f} vs avg {avg_range:.4f} ({pct:.0f}%) — "
...
def has_clean_pullback(df_m1, trend, n=2):
    ...
    Note: ETH spreads in USD are much smaller than BTC spreads in absolute terms,
    but can be relatively wide (%) during low-liquidity periods.
    """
    if len(df_m1) < n + 3:
        return False
    ...
    for _, c in pullback.iterrows():
        ...
        if trend == 'BUY':
            ...
            wick_clean   = (upper_wick / candle_range) <= 0.05
