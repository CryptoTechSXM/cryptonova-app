...
def get_daily_atr(symbol, period=14):
    ...
    unavailable — common for stock CFDs (NFLX, NFLX.s etc.) where
    brokers only carry intraday bars.
    """
    def _compress(df, bars_per_day):
        """Group intraday bars into daily buckets and return daily OHLC."""
        df = df.copy()
    ...
    df = None
    # Try each timeframe in descending order until one returns data
    for tf, bars_per_day, description in [
        (1440, 1,  'D1'),
        (240,  6,  'H4'),
        (60,   24, 'H1'),
        (15,   32, 'M15'),
