"""
lot.py — Dynamic Lot Sizing (BTC HA Bot V3)

Calculates lot size so that a full SL hit costs exactly RISK_PERCENT
of the current account balance.

Example — $97 account, 1% risk, BTC at $85,000:
  Risk amount  = $97 × 1% = $0.97
  ATR          ≈ $300  →  SL distance = 2 × ATR = $600
  Contract     = 1 BTC per lot  →  pip_value = $1 per $1 move per lot
  Lot          = $0.97 / $600 = 0.0016 → rounds to broker min (e.g. 0.01)
...
def calculate_lot_size(symbol: str, sl_distance: float, risk_percent: float = 1.0) -> float:
    ...
    # For most crypto CFDs: contract_size (e.g. 1 BTC) × point value
