# Git Sync Report — 2026-05-15 23:08 UTC (standalone run record)

**Status: NOT RUN.** Same structural blocker as every unattended run since 2026-05-10. This is the 10th standalone failure file for 2026-05-15 (11:10, 13:09, 14:10, 15:09, 16:09, 17:09, 18:09, 19:09, 20:09, 23:08 UTC), on top of the shared `git_sync_report_2026-05-15.md`.
...
## What was checked this run (`date -u` = `Fri May 15 23:08:40 UTC 2026`)

1. **The script is still unreachable.** `python C:\CryptoNite-MT5-Bots\git_sync.py` cannot run from the Cowork sandbox. Only the four child folders are mounted (`0-tg-mt5-bot`, `1-btc-ha-bot`, `6-Quick-Scalp-NAS100`, `CryptoNite-Free-Signals`) plus `outputs`/`uploads`/`.claude`. The parent `C:\CryptoNite-MT5-Bots\` — which holds `git_sync.py` and the `.git/` checkout — is **not** mounted. Re-verified: `ls /sessions/.../mnt/CryptoNite-MT5-Bots` → "No such file or directory".
2. **No sandbox-side git fallback.** `find -maxdepth 4 \( -name git_sync.py -o -name .git -o -name .gitignore \)` returns nothing. None of the four mounted folders is itself a git repo, and none contains a `.gitignore`. There is no local repo or remote the sandbox can act on.
3. **No Windows-side run occurred.** No `_sync_output.txt` and no `sync_done.flag` in `1-btc-ha-bot/` — the `_temp_run_sync.bat` wrapper did not fire from any other source either. `1-btc-ha-bot/heartbeat.txt` held `1778886515.7091563` at the start of this run and was left untouched.
