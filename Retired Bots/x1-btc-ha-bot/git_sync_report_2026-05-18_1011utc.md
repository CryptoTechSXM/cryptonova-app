...
- **Run timestamp (UTC):** 2026-05-18 10:11
...
The script lives at `C:\CryptoNite-MT5-Bots\git_sync.py`. The parent folder `C:\CryptoNite-MT5-Bots` itself is **not** mounted into Cowork's Linux sandbox; only its four bot subfolders are bind-mounted individually:
...
3. Running the script therefore requires driving the Windows desktop (Run dialog or terminal) via the computer-use tool, which requires explicit user approval through an on-screen dialog.
4. This run is unattended — no user is at the machine to approve the access dialog. The `request_access` call for "Command Prompt" was attempted this run and timed out after 180 s, matching every prior unattended run.

## What this run did

- Confirmed mount layout (parent folder still not mounted).
- Confirmed `1-btc-ha-bot` is not a standalone git repo (`git rev-parse --show-toplevel` → fatal: not a git repository).
- Attempted `request_access(["Command Prompt"])` → timed out at 180 s.
