...
def _session_close_ok(config):
    """Block new entries within SESSION_CLOSE_BUFFER minutes of NY (19:00) close."""
...
def check_signal(df_m1, df_m5, df_h1, config):
    ...
    if not SILENT:
        print(f'Price: {price:.2f} | EMA100: {ema_val:.2f} | ATR: {current_atr:.2f}')
    ...
    print(f'[{config.SYMBOL}] SIGNAL: {trend} | Entry={price:.2f} SL={sl:.2f} TP={tp:.2f} | R:R=1:{rr:.1f}')
