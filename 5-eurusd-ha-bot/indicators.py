...
def is_doji(candle, threshold=0.15):
    ...
    if range_ == 0:
        return False
    ratio = body / range_
    print(f"Doji check: Body={body:.5f} Range={range_:.5f} Ratio={ratio:.4f} (max:{threshold})")
...
def is_high_volume_doji(df_m1, lookback=3):
    ...
    Doji is at df_m1.iloc[-3] in strategy.py.
    """
    ...
    print(f"Doji size: {doji_range:.5f} vs avg {avg_range:.5f} ({pct:.0f}%) -- {'OK' if result else 'LOW VOL'}")
    return result
...
def has_clean_pullback(df_m1, trend, n=2):
    ...
    Doji is at df_m1.iloc[-3], so pullback is at df_m1.iloc[-(n+3):-3].
    """
    if len(df_m1) < n + 3:
        return False
