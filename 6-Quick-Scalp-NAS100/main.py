...
def try_box_opening_range():
    ...
    try:
        ...
        if not valid:
            print(f'[{config.SYMBOL}] Box {pct:.0f}% of ATR - outside 20-35% range, no trade today')
            today_box = 'invalid'
            return
        today_box = (box_high, box_low, open_time, daily_atr)
        print(f'[{config.SYMBOL}] Opening box: {box_low:.1f} - {box_high:.1f} | ATR {daily_atr:.0f} | Range {pct:.0f}%')
    except Exception as e:
    ...
    try:
        while True:
            ...
            try:
                ...
                if signal:
                    print(f'[{config.SYMBOL}] {signal["pattern"].upper()} {signal["type"]} | Entry={signal["entry"]:.1f} SL={signal["sl"]:.1f} TP={signal["tp"]:.1f}')
