# Git Sync Report — 2026-05-17 01:13 UTC (standalone run record)
...
Sandbox time: `Sun May 17 01:13:00 UTC 2026`. Roughly 57 minutes after the previous standalone report (`git_sync_report_2026-05-17_0016utc.md`).
...
3. **No Windows-side run occurred since 00:16 UTC.** No `_sync_output.txt`, no `_git_sync_output.log`, no `sync_done.flag` in `1-btc-ha-bot/`. The pre-existing wrappers (`_git_sync_run.bat`, `_temp_run_sync.bat`, `_run_git_sync.bat`) did not fire from any other source in the last hour.
...
5. **Heartbeats still aging — no bot process is touching them.**
   - `1-btc-ha-bot/heartbeat.txt` mtime `2026-05-15 00:23:53 UTC` → **~48h stale**.
   - `6-Quick-Scalp-NAS100/heartbeat.txt` mtime `2026-05-14 15:07:52 UTC` → **~58h stale**.
   - `0-tg-mt5-bot/heartbeat.txt` mtime `2026-05-11 00:09:48 UTC` → **~145h stale (~6d)**.
...
- **Option 1 (recommended):** move the sync to Windows Task Scheduler, pointing at the existing `1-btc-ha-bot\_temp_run_sync.bat` wrapper. Runs under your Windows account with your existing git credentials, bypasses the Cowork sandbox.
...
**Until one of those is in place, please pause or delete this Cowork scheduled task** — each run only produces another no-op report like this one. The aging heartbeats (48h–145h stale, one fully missing) also indicate the bots themselves are not running on the Windows side; worth a look the next time you're at the machine.
