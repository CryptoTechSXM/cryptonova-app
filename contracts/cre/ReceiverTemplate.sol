...
abstract contract ReceiverTemplate is IReceiver, Ownable {
  ...
  function onReport(bytes calldata metadata, bytes calldata report) external override {
    ...
    if (s_expectedWorkflowId != bytes32(0) || s_expectedAuthor != address(0) || s_expectedWorkflowName != bytes10(0)) {
      ...
      if (s_expectedWorkflowId != bytes32(0) && workflowId != s_expectedWorkflowId) revert InvalidWorkflowId(workflowId, s_expectedWorkflowId);
      if (s_expectedAuthor != address(0) && workflowOwner != s_expectedAuthor) revert InvalidAuthor(workflowOwner, s_expectedAuthor);
  ...
  function setExpectedWorkflowName(string calldata _name) external onlyOwner {
    ...
    for (uint256 i = 0; i < 10; i++) first10[i] = hexString[i];
